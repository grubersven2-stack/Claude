...
if mod(step, 200) == 0
    ...
    %% STEP 1: DETERMINE OPERATING MODE AND ENERGY FACTOR
%{
    if state.temperature < params.T_equilibrium_target
        current_energy_factor = params.energyFactor_heating;    % 30%
        mode_label = 'HEATING';
    else
        current_energy_factor = params.energyFactor_circulation; % 81.3%
        mode_label = 'CIRCULATION';
    end
%}

    %% Calculate base energy flows
    ...
    Q_input_net = Q_input_total * current_energy_factor - Q_losses;
...
if mod(step, 200) == 0
    ...
    %% STEP 5: USE EXCESS ENERGY FOR THERMAL MASS HEATING/COOLING (FIXED!)
    % FIXED: Properly handle negative Q_excess (condensation)
    evapRate_thermal_mass = Q_excess / hfg;  % Can be positive (evap) or negative (condensation)
    evapRate = evapRate_thermal_mass;  % NH3 circulation is a closed loop - no net mass change
    massEvaporated = evapRate * params.timeStep;
